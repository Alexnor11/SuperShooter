...
public class Sight : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for(int i = 0; i < colliders.Length; i++)
        {
            ...
            Vector3 directionToContriller = Vector3.Normalize(collider.bounds.center - transform.position);

            float angelToCollider = Vector3.Angle(transform.forward, directionToContriller);

            if(angelToCollider < angle)
            {
